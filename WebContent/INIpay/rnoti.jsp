...
String file_path = "/carcaro/pay_log";  //로그를 기록할 디렉터리
//**********************************************************************************
...
if("118.129.210.25".equals(addr) || "211.219.96.165".equals(addr)) //PG에서 보냈는지 IP로 체크 
{
	...
	settle.setOid(P_OID);
	settle.setTid(P_TID);
	settle.setAmount(P_AMT);
	settle.setMethod(P_TYPE);
	settle.setSettleTime(P_AUTH_DT);
	...
	***********************************************************************************/
	...
	{			
		...
		//ret = dao.appLogSettlement(P_OID, P_AUTH_DT, P_STATUS, P_TYPE, P_TID, P_FN_CD1, P_FN_CD2, P_FN_NM, P_AMT, P_UNAME, P_RMESG1, P_RMESG2, P_NOTI, P_AUTH_NO);
...
%><%!
	...
	{
		try {
			connPool = new ConnectionPool(MainServlet.driver,
					MainServlet.connection, MainServlet.userId, MainServlet.userPw, 5);
			
			dao = new SettlementDAO(connPool);
			//dao.resultSettlement();
		} catch(Exception e) {
			e.printStackTrace();
		}		
	}
